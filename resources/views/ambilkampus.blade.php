...
<head>
  ...
  <title>Collsect - Pilih Kampus</title>
...
<body>
  ...
  <header id="header" class="d-flex flex-column justify-content-center">
    ...
    <nav id="navbar" class="navbar nav-menu">
      <ul>
        <li><a href="{{route('welcome')}}" class="nav-link scrollto active"><i class="bx bx-home"></i> <span>Beranda</span></a></li>
      </ul>
    </nav><!-- .nav-menu -->
  ...
  <section id="hero" class="d-flex flex-column justify-content-center">
    <div class="container" data-aos="zoom-in" data-aos-delay="100">
      ...
      <p>Pilih <span class="typed" data-typed-items="Kampus"></span>Kamu !! </p>
      <br>
      <form action="{{ route('pilihkampus') }}" method="post" >
        @csrf
        <select name="kampus1">
          @foreach($alternatif as $data)
            <option id="{{$data['kode_alternatif']}}"> {{$data['nama_alternatif']}} </option>
          @endforeach
        </select>
        <br>
        <select name="kampus2">
